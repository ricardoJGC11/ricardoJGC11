## Sobre mí

Estudiante de Ingeniería en Sistemas y Computación.

Actualmente desarrollo aplicaciones web y APIs, disfrutando especialmente el backend, las bases de datos y el diseño de software.

**Tecnologías principales:** Java, Python (FastAPI), C#, JavaScript, PostgreSQL y MySQL.

---

## Proyectos Destacados

### Gestor CMMS

Sistema de Gestión de Mantenimiento Computarizado desarrollado con **FastAPI**, **React + Vite** y **PostgreSQL**. Permite administrar activos, órdenes de trabajo y el historial de mantenimiento de una planta mediante una arquitectura cliente-servidor.

### BotE-Commerce

Sistema de comercio electrónico desarrollado con **Python**, **FastAPI**, **PostgreSQL** y un **Bot de Telegram**. Permite gestionar productos y pedidos a través de una API REST integrada con Telegram.

---

Tecnologías y Herramientas
Lenguajes

<p align="left"> <img src="https://skillicons.dev/icons?i=python,java,cs,js" /> </p>

Frameworks y Librerías

<p align="left"> <img src="https://skillicons.dev/icons?i=fastapi,react,angular,vite" /> </p>

Bases de Datos

<p align="left"> <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb" /> </p>

Herramientas

<p align="left"> <img src="https://skillicons.dev/icons?i=git,github,vscode,maven" /> </p>

## Contacto

- LinkedIn: https://www.linkedin.com/in/ricardo-javier-godoy-castro-21a8892ba/
- Correo: ricardo.godoy.7575@gmail.com

---

